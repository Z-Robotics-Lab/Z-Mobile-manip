...
def test_close_target_and_nonempty_verification_are_width_based():
    ...
    # A real held object blocks the aperture ABOVE the commanded close target --
    # that stall gap is the primary contact evidence (live 2026-07-24: a genuine
    # bottle hold sat 1.1mm above the commanded target).  Force alone cannot
    # prove a hold: an empty crush to the commanded width registered -0.18N, more
    # than the -0.114N of a true soft hold, so the force floor sits above both
    # and the stall gap does the discriminating.
    soft_hold_with_stall_gap = EXECUTOR.GripperFeedback(
        0.0285,
        -0.114,
        1.0,
        "width",
    ...
    EXECUTOR.verify_nonempty_grasp(
        soft_hold_with_stall_gap,
        0.03,
        minimum_force_n=0.0,
        commanded_close_target_m=0.026,
    )
    # The recorded false positive: fingers crushed to the commanded width (no
    # stall gap) at -0.18N must now be rejected as empty, not lifted as a hold.
    empty_crush_to_target = EXECUTOR.GripperFeedback(
        0.0265,
        -0.18,
        1.0,
        "width",
