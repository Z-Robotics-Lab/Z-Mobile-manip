...
# Install the single-owner Go2W SPORT bridge on the NUC, the fixed posture
# relay on this workstation, and the on-demand PiPER reactive-view executor.
# The arm service is deployed but explicitly disabled and stopped; only an
# operator-started Live depth-servo session may acquire that CAN owner.
...
"${SSH[@]}" '
  ...
  systemctl --user disable --now z-manip-go2w-base-control.service >/dev/null 2>&1 || true
  systemctl --user disable --now z-mobile-manip-piper-reactive-view.service >/dev/null 2>&1 || true
  systemctl --user restart z-manip-piper-passive-feedback.service
  systemctl --user is-active --quiet z-manip-piper-passive-feedback.service
  systemctl --user enable z-mobile-manip-go2w-reactive-live.service
  systemctl --user restart z-mobile-manip-go2w-reactive-live.service
'
