...
Extracted sibling modules (behavior-preserving, names re-imported here so the
existing ``go2w_planning_control.<name>`` surface is unchanged):
  * ``go2w_runtime_state`` -- Stage 1: ``RuntimeStateError``,
    ``validate_runtime_state`` and the runtime content-limit constants.
"""
...
# monkeypatches it on this module and RuntimeStateReader reads it as a global.
